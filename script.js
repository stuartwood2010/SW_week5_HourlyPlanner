...
for (let i = 0; i < weeklyPlanEl.length; i++) {
    let meetings = localStorage.getItem('events');
    let hourEl = $('<li>');
    hourEl.text(weeklyPlanEl[i].text);
    hourEl.attr('value', i);
    hourEl.addClass('hour row col-1');
    hourEl.css({'list-style-type': 'none', 'font-weight': 'bold'});
    let contentEl = $('<input>');
    contentEl.attr('type', 'text');
    contentEl.addClass('time-block row description col-10');
...
if (currentHour < weeklyPlanEl[i].value) {
    ...
    saveEl.on('click', function saveEvent(event) {
        event.preventDefault();        
        localStorage.setItem('events', meetings);
});
}
